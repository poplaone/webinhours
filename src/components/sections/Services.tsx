      ...
      <div className="container mx-auto">
            ...
            >
              <div className="bg-card border border-border/40 rounded-xl p-6 h-full transition-all duration-300 hover:shadow-xl hover:border-[#8B5CF6]/30 hover:-translate-y-1">
                <div className="flex items-center gap-3 mb-4">
                  <div className={`w-12 h-12 rounded-lg flex items-center justify-center`} style={{ backgroundColor: `${service.activeColor}15` }}>
                    <service.icon className="h-6 w-6" style={{ color: service.activeColor }} />
                  </div>
                  <div>
                    <h3 className="text-xl font-semibold group-hover:text-[#8B5CF6] transition-colors">{service.title}</h3>
                    <span className="text-sm font-medium" style={{ color: service.activeColor }}>{service.price}</span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  {service.features.map((feature, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-sm">
                      <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: service.activeColor }}></div>
                      <span>{feature}</span>
                    </div>
                  ))}
                </div>
        ...
        >
          <div className="bg-gradient-to-r from-[#8B5CF6]/10 to-[#A78BFA]/10 border border-[#8B5CF6]/20 rounded-xl p-8">
            ...
            <motion.button 
              className="bg-[#8B5CF6] hover:bg-[#7C3AED] text-white px-8 py-3 rounded-lg font-medium transition-all duration-300 hover:scale-105"
