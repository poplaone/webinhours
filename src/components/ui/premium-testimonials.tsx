...
export function PremiumTestimonials() {
  ...
  return <section id="testimonials" className="relative py-20 bg-card/30 backdrop-blur overflow-hidden">
      {/* Background Effects */}
      <div className="absolute inset-0">
        <motion.div className="absolute inset-0 bg-gradient-to-br from-[#8B5CF6]/[0.08] via-[#A78BFA]/[0.05] to-[#DDD6FE]/[0.08]" animate={{
        ...
        <motion.div className="absolute top-1/3 left-1/5 w-72 h-72 bg-[#8B5CF6]/15 rounded-full blur-3xl" animate={{
        x: [0, 150, 0],
        y: [0, 80, 0],
        scale: [1, 1.2, 1]
      }} transition={{
        ...
        <motion.div className="absolute bottom-1/3 right-1/5 w-80 h-80 bg-[#A78BFA]/15 rounded-full blur-3xl" animate={{
        x: [0, -100, 0],
        y: [0, -60, 0],
        scale: [1, 1.3, 1]
      }} transition={{
        ...
        {[...Array(12)].map((_, i) => <motion.div key={i} className="absolute w-1 h-1 bg-foreground/30 rounded-full" style={{
        left: `${15 + i * 7}%`,
        top: `${25 + i * 5}%`
      }} animate={{
        y: [0, -50, 0],
      ...
      <motion.div ref={containerRef} className="relative z-10 container mx-auto px-4" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{
      once: true,
      margin: "-100px"
    }}>
        <motion.div className="text-center mb-16" variants={fadeInUp}>
          <motion.div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-card/50 border border-border/40 backdrop-blur-sm mb-6" whileHover={{
          scale: 1.05,
          ...
          <motion.p variants={fadeInUp} className="text-muted-foreground max-w-3xl mx-auto text-sm">
        ...
        <div className="hidden lg:block mb-16">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
            {testimonials.slice(0, 3).map((testimonial, index) => <motion.div key={index} initial={{
            opacity: 0,
            y: 30
          }} whileInView={{
            opacity: 1,
            ...
            once: true
                ...
                <div className="bg-card border border-border/40 rounded-xl p-6 h-full hover:shadow-xl hover:border-[#8B5CF6]/30 transition-all duration-300 hover:-translate-y-1">
                  ...
                  <div className="flex items-center gap-1 mb-3">
                    {[...Array(5)].map((_, i) => <div key={i} className="w-4 h-4 text-yellow-400 fill-current">★</div>)}
                  </div>
                  
                  <p className="text-foreground mb-4 leading-relaxed">{testimonial.text}</p>
                  
                  <div className="space-y-2 mb-4">
                    {testimonial.results.map((result, idx) => <div key={idx} className="flex items-center gap-2 text-sm">
