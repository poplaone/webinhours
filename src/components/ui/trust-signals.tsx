...
export const TrustSignals: React.FC<TrustSignalsProps> = ({
  ...
  if (variant === 'grid') {
    return <div className={`grid grid-cols-2 md:grid-cols-3 gap-6 ${className}`}>
        {signals.map((signal, index) => <motion.div key={index} className="text-center p-4 bg-card/50 backdrop-blur border border-border/40 rounded-lg hover:shadow-lg transition-all duration-300" initial={{
        opacity: 0,
        y: 20
      }} whileInView={{
        opacity: 1,
  ...
  return <div className={`flex flex-wrap justify-center gap-8 ${className}`}>
      {signals.map((signal, index) => <motion.div key={index} className="flex items-center gap-2 text-sm" initial={{
      opacity: 0,
      x: -20
    }} animate={{
      opacity: 1,
