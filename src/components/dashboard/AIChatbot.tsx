...
export const AIChatbot: React.FC<AIChatbotProps> = ({ onTemplateRecommend }) => {
  ...
  const generateBotResponse = (userInput: string): Message => {
    ...
    if (input.includes('portfolio') || input.includes('creative')) {
      return {
        ...
        timestamp: new Date(),
        suggestions: [
          "CreativeStudio - Portfolio",
          "DesignPro - Agency Website",
          "ArtistSpace - Creative Portfolio",
    ...
    >
      <Card className={`${isExpanded ? 'h-full' : 'min-h-[600px]'} flex flex-col bg-transparent border-0 shadow-none`}>
        ...
        <CardContent className="flex-1 flex flex-col p-0">
          ...
          <div className="pt-4 pb-2 border-t border-muted flex items-center w-full max-w-[340px] mx-auto">
            <input
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              placeholder="Ask me anything..."
              onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()}
              className="flex-1 bg-transparent border border-primary rounded-full px-4 py-2 text-primary placeholder:text-muted-foreground focus:outline-none"
            />
            <button
              onClick={handleSendMessage}
              disabled={!inputValue.trim() || isTyping}
              className="ml-2 bg-primary text-white rounded-full p-3 shadow-md hover:bg-primary/80 transition"
            >
